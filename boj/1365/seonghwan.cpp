...
1. 전선이 꼬이지 않아야 한다는 말은 상대편 전봇대의 번호가 무조건 증가하는 방향으로 연결되어야 한다는 뜻
2. 따라서 증가하다가 감소하는 포인트의 숫자는 무시한다.
3. 하지만 감소한 지점에서부터 차례대로 증가하는 수열을 구했을 때가 더 길 수 있기 때문에
		(=연결 할 수 있는 전선이 더 많을 수 있기 때문에) 마냥 무시할 수는 없는 법
4. 따라서 이중 for문으로 증가하는 수열이 가장 긴 수열을 고를 수 있다
		(1중 for문 : 시작하는 전봇대 지점, 2중 for문 : 시작 전봇대 지점으로부터 증가하는 수열 구하기)
5. 하지만 n^2 시간 복잡도로 100,000 * 100,000의 연산을 해야할 수 있으므로 안됨.
6. 증가하다가 감소하는 수를 만났을 때 만약 앞에서 현재 마주친 수보다 크고 직전에 만난 수보다는 작은 수들을 아예
...
7. 위 내용을 기둥의 갯수만큼 반복하면 n * logn의 시간 복잡도로 시간초과 안남
8. 위 작업을 하게되면 전선의 오름차순 중 가장 긴 수열이 나오진 않는다(7번에 만약에라는 가정으로 현재 검사중인
		기둥 번호를 증가하는 기둥 수열의 중간에 임의로 삽입을 해 버렸기 때문). 하지만 수열의 길이를 보고자 하는
		것이므로 길이만 보기위함이면 상관없음
9. 이렇게 증가하는 방향으로 연결된 전선들 중 최대 갯수를 찾으면 전체 전선의 길이에서 이걸 빼 주면 잘라야 할 전선이 됨
*/
