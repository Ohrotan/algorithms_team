/*
    answer += 'O'를 answer = answer + 'O'라고 해서 시간초과 났었음...약 2시간 잡아먹음....다른 하이레벨 언어 쓰는 사람들은 조심해야할듯. 이유는
    answer + 'O' 연산 결과를 answer에 덮어 쓸 때,answer크기보다 더 큰 저장공간이 필요하기 때문에 realloc가 일어나서 다시 할당 되어야
    하기 때문에 매번 크기 재할당을 하기엔 이에대한 오버헤드가 큼(+=나 append 사용시, 기존 크기보다 넘어서게 되면 기존 크기의 2배의
    크기로 넉넉하게 할당해서 재할당 횟수 = log(n)으로 줄어듦. answer = answer + 'O'는 재할당 횟수 = n)
*/
